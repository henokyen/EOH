  ...
  struct Params
  {
    ...
    struct dnn
    {
     ...
     struct sampled
      ...
      static const size_t min_nb_conns = 11;
      static const size_t max_nb_conns = 20; //nb_outputs * 10;
      static const float m_rate_add_conn = 0.2f;
      static const float m_rate_del_conn = 0.2f;

      static const float m_rate_change_conn = 0.0f;
      static const float m_rate_add_neuron = 0.05f; //Henok: increased the rate of adding neurons: complisfication 
...
int test=0;
      ...
      //std::string fname="/project/EvolvingAI/hmengist/7.25/noweight_andxorand/nn"+_postFix+".svg";
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
    std::string fname="/project/EvolvingAI/hmengist/7.25/andxorand/PCCnorm/nn"+_postFix+".svg";
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/7.25/andxorand/PCCnorm/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/7.25/andxorand/PCCnorm/nn"+_postFix+".dot");
    indiv.nn().write(ofs);

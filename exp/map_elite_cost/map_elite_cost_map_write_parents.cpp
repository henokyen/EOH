...
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/map_elite_cost/map_elite_cost.cpp"
#include <iostream>
#include <bitset>
#include <algorithm>
 ...
 struct visualisation
    ...
    struct pop
    {
     ...
     //static const unsigned nb_gen = 5001;
      ...
      #endif
      static const int dump_period = 1000;//1000;
    ...
    struct ea
    {
      ...
      static const size_t res_x = 256;
      static const size_t res_y = 256;
      static const size_t res_z = 256;
    };
    struct rand_obj
    {
...
SFERES_FITNESS(FitMap, sferes::fit::Fitness)
{
  public:
    FitMap() : _desc(3) { }
    // to normalize modularity
    float normalize (float  mod)
    {
      float md;
      md = (mod +1.0) / 2.0;
      return md;
  ...
  string NumberToString ( T Number )
    ...
    void set_desc(float x, float y, float z)
        {
          //std::cout<<"x1 before: "<<x1<<" x1 after:  ";
          //x1 = normalize(x1);
          //std::cout<<x1<<std::endl;
          assert(y >= 0);
          //assert(x1 >= 0);//modularity can be negative,that is why it is commented
          assert(x <= 1);
          assert(y <= 1);
          assert(_desc.size() >= 3);
          _desc[0] = x;
...
//std::cout<<"did u call me?"<<std::endl;
...
//    this->set_desc(_mod, _grc);
     ...
     std::string fname="/project/EvolvingAIs/hmengist/newmega/3hxorand3hxor/PA/log/nn"+_postFix+".svg";
     _optlength = ind.gen().compute_optimal_length(0, write_svg,fname);
     //_optlength=_length;
  this->set_desc(_mod, _grc,_optlength);
...
bool tosvg=false;
      ...
      {
       std::fstream mapnets;
       std::string fn="/project/EvolvingAI/hmengist/7.25/map_elite/map_points_valid.dat";
       mapnets.open(fn.c_str(),std::fstream::out | std::fstream::app);
...
mapnets.close();
      ...
      //bool write_svg = (this->mode() == fit::mode::view);
      //std::string fname="/project/EvolvingAI/hmengist/7.25/map_elite/map_networks/"+NumberToString(_grc)+"_"+NumberToString(_mod)+"_nn.svg";
      //_optlength = ind.gen().compute_optimal_length(0, write_svg,fname,(1-err1/norm));
//string fdot="/project/EvolvingAI/hmengist/7.25/map_elite/map_networks/"+NumberToString(_grc)+"_"+NumberToString(_mod)+"_nn.dot";
  ...
  for (size_t i = 1; i < nb_layers; ++i) //for each layer 
   {
      ...
      if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
	{
     ...
     //deb.open("/project/EvolvingAI/hmengist/7.25/andxorand/PAsampling/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    //cout<<endl;        
    std::string fn="/project/EvolvingAI/hmengist/7.25/map_elite/map_networks/"+NumberToString(_grc)+"_"+NumberToString(_mod)+"solvent.txt";
    sol.open(fn.c_str(),std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::string fdot = "/project/EvolvingAI/hmengist/7.25/map_elite/map_networks/nn"+NumberToString(_grc)+"_"+NumberToString(_mod)+".dot";
    std::ofstream ofs(fdot.c_str());
    indiv.nn().write(ofs); 
}
...
tes.open("/project/EvolvingAI/hmengist/7.25/andxorand/PAsampling/test.txt",std::fstream::out | std::fstream::app);

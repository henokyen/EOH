...
namespace sferes
{
  namespace stat
  {
	...
	for (size_t i = 0; i < _xs; ++i)
	  for (size_t j = 0; j < _ys; ++j)
           for (size_t k = 0; k < _zs; ++k)
	      if (array[i][j][k] && p_array[i][j][k])
	      {
		point_t p =ea.get_point(p_array[i][j][k]);
                //point_t p = _get_point(p_array[i][j]);
		size_t x = round(p[0] * _xs);
		size_t y = round(p[1] * _ys);
                size_t z = round(p[1] * _zs);
		ofs << i / (float) _xs
		    << " " << j / (float) _ys
                    << " " << k / (float) _zs
  ...
  //               std::cout<<"are u writing? if there is a point"<<endl;
                   _avg_fitness.push_back(array[i][j][k]->fit().value());
                  }

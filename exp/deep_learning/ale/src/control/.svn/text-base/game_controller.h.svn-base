...
class GameController {
    ...
    ************************************************************************* */
    public:
        ...
        void systemReset();
	void reset_console() { p_global_event_obj->set(Event::ConsoleReset, 1); }

	RomSettings* rom_settings() { return m_rom_settings; }
	// JBM
	int screen_width() const { return i_screen_width; }
	int screen_height() const { return i_screen_height; }      
	void export_screen() const;
	const IntMatrix& screen_matrix() const { return _screen_matrix; }
	void copy_framebuffer();
	void setNumSystemResetSteps(int k) { p_num_system_reset_steps = k; }
    protected:

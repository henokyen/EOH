...
float OSystem::mainLoop(InternalController *game_controller,
                        size_t nb_frames,
			int nb_reset,
                        bool export_screen)
{
  myGameController = game_controller;
  game_controller->setNumSystemResetSteps(nb_reset);
  float reward = 0;
  game_controller->resetReward();
  for (size_t i = 0; i < nb_frames && !game_controller->has_terminated(); ++i)
  {    
    myGameController->update();
    console().mediaSource().update();
    
    if (export_screen)
      game_controller->export_screen();

    reward = std::max(reward, (float)game_controller->getReward());
    
  }
  game_controller->end_episode();

...
#define PR25
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/comb_evo.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
 ...
 void eval(Indiv& ind)
     {
      ...
      for (size_t s = 0; s < k; ++s)
      {
         ...
         for (size_t i = 0; i < ind.gen().get_depth(); ++i)
            ind.nn().step(inputs);
            // behaviors
          std::vector<bool> comb_res;
         for (size_t i = 0; i < Params::dnn::nb_inputs; i+=2)
          comb_res.push_back(((bool)(inputs[i] > 0)) || ((bool)(inputs[i + 1] > 0)));
         for (size_t i = 0; i < Params::dnn::nb_outputs; ++i) 
            _behavior[s * Params::dnn::nb_outputs + i] = ind.nn().get_outf(i) > 0 ? 1 : 0;
             // errors
          comb_res.push_back(comb_res[0] ^ comb_res[1]);//4
          comb_res.push_back(!(comb_res[2] ^ comb_res[3]));//5
         if (!(comb_res[4] ^ comb_res[5]))
...
//	}
   ...
   //optimal length
      //bool write_svg = (this->mode() == fit::mode::view);
     // std::string fname="/project/evolutionaryRobotics/hmengist/7.25/andxorand/PA/PAorxorequals/nn"+_postFix+".svg";
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PA/PAorxorequals/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PA/PAorxorequals/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
//#endif
  ...
  else if (argc ==3)
  {
   ...
   do{
     ...
     {
        ea.reset();
        cout<<"Doing is again becaue the fitness of best individual is :"<<endl;
     }

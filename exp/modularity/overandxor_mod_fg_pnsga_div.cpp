...
#define PNSGA
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/overandxor_mod.cpp"
 //#define NO_PaRALLEL
#include <iostream>
#include <bitset>
...
SFERES_FITNESS(FitXorXor, sferes::fit::Fitness)
{
  public:
    ...
    void eval(Indiv& ind)
    {
      ...
      for (size_t s = 0; s < k; ++s)
      {
        ...
        for (size_t i = 0; i < Params::dnn::nb_outputs; ++i)
         ...
         // errors
         and_res.push_back(and_res[0] ^ and_res[1]);//3
         and_res.push_back(and_res[1] ^ and_res[2]);//4
...
#ifdef MOD
      this->_objs.push_back(1-_mod);//discouraging modularity
#endif
#ifdef HE
    ...
    bool write_svg = (this->mode() == fit::mode::view);
    std::string fname="/project/EvolvingAI/hmengist/7.25/overandxor_mod/overandxormod100/nn"+_postFix+".svg";
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/7.25/overandxor_mod/overandxormod100/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/7.25/overandxor_mod/overandxormod100/nn"+_postFix+".dot");
    indiv.nn().write(ofs);

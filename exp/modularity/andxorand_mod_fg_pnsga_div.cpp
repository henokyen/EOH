...
int test=0;
      ...
      std::string fname="/project/evolutionaryRobotics/hmengist/7.25/noweight_andxorand/nn"+_postFix+".svg";
...
//	}
   ...
   //optimal length
      //bool write_svg = (this->mode() == fit::mode::view);
    //std::string fname="/project/evolutionaryRobotics/hmengist/7.25/map_elite/subproblem_network/nn"+_postFix+".svg";
  ...
  for (size_t i = 1; i < nb_layers; ++i) //for each layer 
   {
      ...
      if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
	{
     ...
     //deb.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PAsampling/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/map_elite/subproblem_network/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/map_elite/subproblem_network/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
tes.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PAsampling/test.txt",std::fstream::out | std::fstream::app);

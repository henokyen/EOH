...
#define PNSGA
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/combnation_evo.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
      std::string fname="/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PA/nn"+_postFix+".svg";
...
template <typename T> 
void combinationevos_recursive( vector<T> &elems, unsigned long req_len,
    ...
    if ((elems.size() - margin) < (req_len - depth))
     ...
     for (unsigned long ii = margin; ii < elems.size(); ++ii) {
        pos[depth] = ii;
      combinationevos_recursive(elems, req_len, pos, depth + 1, ii + 1,ans);
   }
    return;
  }
template <typename T> 
void combinationevos( vector<T> &elems, unsigned long req_len, vector <T>& ans)
 {
   assert(req_len > 0 && req_len <= elems.size());
   vector<unsigned long> positions(req_len, 0);
   combinationevos_recursive(elems, req_len, positions, 0, 0,ans);
 }
         ...
         for (size_t k = 1; k < nb_layers-1; ++k)
          {
            if (simplayer[k].size() > 1)
             {
                combinationevos<typename I::nn_t::vertex_desc_t>(simplayer[k],2,pairof);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PA/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PA/nn"+_postFix+".dot");
    indiv.nn().write(ofs);

...
#define PR100
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/overand_mod.cpp"
 //#define NO_PaRALLEL
#include <iostream>
#include <bitset>
...
SFERES_PARAMS(
  struct Params
  {
    struct visualisation
      {
         static const size_t  max_modularity_split = 8;
      };
...
static float pressure;
#ifdef PR10
      SFERES_ARRAY(float, obj_pressure, 1.0, 0.1, 1.0,1.0);
#elif defined(PR25)
      SFERES_ARRAY(float, obj_pressure, 1.0, 0.25, 1.0,1.0);
#elif defined(PR50)
      SFERES_ARRAY(float, obj_pressure, 1.0, 0.5, 1.0,1.0);
#elif defined(PR100)
      SFERES_ARRAY(float, obj_pressure, 1.0, 1.0, 1.0,1.0);
#else
      SFERES_ARRAY(float, obj_pressure, 1.0, 0.1, 1.0,1.0);
#endif
    };
    ...
    struct dnn
    {
      ...
      static const float m_rate_change_conn = 0.0;
      ...
      struct spatial
      {
        SFERES_ARRAY(int, layers, 5, 4, 3, 2, 1);
    ...
    struct fitness
    {
      static const size_t b_size = 32 * Params::dnn::nb_outputs;
    };
  };
  ) // end of params
  ...
  public:
    ...
    void eval(Indiv& ind)
    {
      ...
      for (size_t s = 0; s < k; ++s)
      {
        ...
        for (size_t i = 0; i < Params::dnn::nb_inputs; i += 1)
          and_res.push_back(((bool)(inputs[i] > 0)) &&  ((bool)(inputs[i + 1] > 0)));
        for (size_t i = 0; i < Params::dnn::nb_outputs; ++i)
          _behavior[s * Params::dnn::nb_outputs + i] = ind.nn().get_outf(i) > 0 ? 1 : 0;
         // errors
         and_res.push_back(and_res[0] && and_res[1]);//4
         and_res.push_back(and_res[1] && and_res[2]);//5
         and_res.push_back(and_res[2] && and_res[3]);//6
         and_res.push_back(and_res[4] && and_res[5]);//7
         and_res.push_back(and_res[5] && and_res[6]);//8

        if (and_res[7] && and_res[8])
	 ...
	 _grc = hierar::Get_GRC(gsimp);
...
#ifdef MOD
      this->_objs.push_back(1-_mod);//discouraging modualrity
#endif
#ifdef HE
      ...
      std::string fname="/project/evolutionaryRobotics/hmengist/mega/3hxor/PA/log/nn"+_postFix+".svg";
    ...
    //bool write_svg = (this->mode() == fit::mode::view);
    //std::string fname="/project/evolutionaryRobotics/hmengist/newmega/gradual50/overPAG/nn"+_postFix+".svg";
    //_optlength = ind.gen().compute_optimal_length(0, write_svg,fname);
...
void ans_subproblem(std::vector<float>& inputs,std::vector<bool>& xorand_res)
  {
       ...
       for (size_t i = 0; i < inputs.size()-1; i += 1)
          xorand_res.push_back(((bool)(inputs[i] > 0)) && ((bool)(inputs[i + 1] > 0)));//xorand_res[0,1,2]
        
          xorand_res.push_back(xorand_res[0] ^ xorand_res[1]);//xorand_res[3]
          xorand_res.push_back(xorand_res[1] ^ xorand_res[2]);//xorand_res[4]
          xorand_res.push_back(xorand_res[3] && xorand_res[4]);//xorand_res[5]
  }
template<typename I>
void get_layer(I& indiv, std::vector<std::vector<typename I::nn_t::vertex_desc_t>> & simplayer)
{
   cout<<"Inside get_layer method"<<endl;
          ...
          for (size_t j = 0; j < Params::dnn::spatial::layers(i); ++j)
            {
              ...
              {
              simplayer[i].erase(simplayer[i].begin()+(j-1));
              }
            }
        }
}
template<typename I>
void testsum_all(I& indiv,std::set<std::pair<int,string>>& results,int sub,
          std::vector<typename I::nn_t::vertex_desc_t> all,             
	      typename I::nn_t::vertex_desc_t com)
{
  string Aid,Bid;
  int sum_all = 0;
  ...
  std::fstream tes;
  tes.open("/project/evolutionaryRobotics/hmengist/newmega/gradual50/overPAG/test.txt",std::fstream::out | std::fstream::app);
  tes<<"Input"<<"\t";
  for (unsigned i=0; i<all.size(); i++){
   tes<<indiv.nn().get_graph()[all.at(i)].get_id()<<"\t";
  }
  tes<<"Sum_"<<indiv.nn().get_graph()[com].get_id()<<"\t"<<"sub="<<sub<<"\n";
  for (size_t s = 0; s < 256; ++s)
   {
		...
		for (unsigned i=0; i<all.size(); i++){
		  sum_all += indiv.nn().get_graph()[all.at(i)].get_current_output()* get_weightVC(indiv,all.at(i),com);
       	  }
		for (unsigned i=0; i<all.size(); i++){
		 tes<<indiv.nn().get_graph()[all.at(i)].get_current_output()<<"("<<get_weightVC(indiv,all.at(i),com)<<")"<<"\t";
       	  }
       values = make_pair(andxorand_res[sub],sum_all);
	   tes<<sum_all<<"\t"<<andxorand_res[sub]<<"\n";
      ...
      if (results.size()==0)
          {
			...
			for (unsigned i=0; i<all.size(); i++){
              indiv.nn().get_graph()[all.at(i)].set_solve(true,sub);
			 }
         }
      else
         {
...
template<typename I>
void sub_problem_decom (I& indiv )
{
  float number_problems = 5.0;
  ...
  for (size_t i = 1; i < nb_layers; ++i)
   {
     for (size_t j = 0; j < simplayer[i].size(); ++j) // for every neuron of a layer 
    {
     ...
     if (!graph[v].is_input() && gsip[v].get_removed()!= 1)
      ...
      deb.open("/project/evolutionaryRobotics/hmengist/newmega/gradual50/debug.txt",std::fstream::out | std::fstream::app);
    deb<<"neuron: "<<indiv.nn().get_graph()[v].get_id()<<"\t"<<"Sub_pro= :"<<t<<endl;
   for (int k=0;k<threshold.size();k++)
      deb<<threshold.at(k).second<<"\t"<<threshold.at(k).first<<endl;
       deb.close();
}
      threshold.clear();
      if (solved) break;
 ...
 if (solved) {solved = false ;continue ;}
  else  //not solved by any of the single neurons on each layer, check for pair of neurons     
       {
         ...
         std::vector<typename I::nn_t::vertex_desc_t> all;
		 typedef typename I::nn_t::vertex_desc_t Vertex;
         typename I::nn_t nn_simp_temp = indiv.gen();
         indiv.gen().simplified_nn(indiv.gen());
         typename I::nn_t::graph_t gsip = indiv.gen().get_graph();
         std::set<string>::iterator it;
         for (size_t k = 2; k < nb_layers; ++k)
          {
           
                //combinations<typename I::nn_t::vertex_desc_t>(simplayer[k],2,pairof);
	        for (int j=0;j<simplayer[k].size();j++)
                {
	         v = simplayer[k][j];
		 if (!graph[v].is_input() && gsip[v].get_removed() == 0){
                  BGL_FORALL_INEDGES_T(v,e,graph,typename I::nn_t::graph_t) 
                    {
                      Vertex src = source (e,graph);
                      if(gsip[src].get_removed() == 0) 
		          all.push_back(src);
		    }
		}
		else 
		 continue ;
		 //for each commom node they are connected to
                 testsum_all(indiv,results,t,all, v);
		 all.clear();
                }
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/newmega/gradual50/overPAG/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/newmega/gradual50/overPAG/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
  ...
  for (size_t i = 1; i < nb_layers; ++i) //for each layer  
   {
     for (size_t j = 0; j < simplayer[i].size(); ++j)  //for every neuron of a layer  
      {
         ...
         v = simplayer[i][j];
         if (!indiv.nn().get_graph()[v].is_input() && gsip[v].get_removed()!= 1)
          {
            for (size_t s = 0; s < 16; ++s)
             {
               std::bitset<4> in(s);
     ...
     //deb.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PA/debug.txt",std::fstream::out | std::fstream::app);
         ...
         for (size_t k = 1; k < nb_layers-1; ++k)
          {
            if (simplayer[k].size() > 1)
             {
                for (int r =0 ; r < simplayer[k].size();r++)
                    comb.push_back(r);
                combinations<int>(comb,2,pairof);
                for (int i=0;i<pairof.size();i=i+2)
                 {
                    towhich_connected (gsip,simplayer[k][pairof[i]],Aconn);
                    towhich_connected (gsip,simplayer[k][pairof[i+1]],Bconn);
                  ...
                  if (!comm.empty())
                    {
//                      cout<<"the pair to be tested "<<gsip[simplayer[pairof[i]]].get_id()<<" "<<gsip[simplayer[pairof[i+1]]].get_id()<<endl;
                      BOOST_FOREACH(typename I::nn_t::vertex_desc_t cnode,comm) //for each commom node they are connected to
                        {
                           testsum_AB(indiv,results,t,simplayer[k][pairof[i]],simplayer[k][pairof[i+1]],cnode);
                        }
                     comm.clear();
                     Aconn.clear();
                     Bconn.clear();
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/overand/PH25/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/overand/PH25/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
tes.open("/project/evolutionaryRobotics/hmengist/7.25/overand/PH25/test.txt",std::fstream::out | std::fstream::app);
...
std::vector<float> inputs(4);
std::fstream sol;
sol.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PH25/nPA_118/nn0336.txt",std::fstream::out | std::fstream::app);

...
#define DIV
#define GRADUAL
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/allands.cpp"
 //#define NO_PARALLEL
#include <iostream>
#include <bitset>
...
static float pressure;
    ...
    struct dnn
    {
      ...
      struct spatial
      {
        SFERES_ARRAY(int, layers, 8, 4, 2, 1);
    ...
    void eval(Indiv& ind)
    {
      ...
      for (size_t s = 0; s < k; ++s)
      {
        ...
        for (size_t i = 0; i < Params::dnn::nb_outputs; ++i)
         ...
         and_res.push_back(and_res[0] && and_res[1]);//and_res[4]
         and_res.push_back(and_res[2] && and_res[3]);//and_res[5]

        if (and_res[4] && and_res[5])
...
void ans_subproblem(std::vector<float> inputs,std::vector<bool>& and_res)
  {
         for (size_t i = 0; i < inputs.size(); i += 2)
          ...
          and_res.push_back(and_res[2] && and_res[3]);//5
          and_res.push_back(and_res[4] && and_res[5]);//6
  }
template<typename I>
void non(I& indiv)
{

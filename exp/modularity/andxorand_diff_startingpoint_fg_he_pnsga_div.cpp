...
#define PNSGA
#define DIV
#line 1 "/home/henok/workspace/sferes/exp/modularity/andxorand_diff_startingpoint.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
  ...
  struct Params
  {
    ...
    struct dnn
    {
     ...
     struct sampled
      ...
      static const size_t min_nb_conns = 11; //to make networks start at different regions
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
      std::string fname="/project/EvolvingAI/hmengist/7.25/andxorand_diif_st_points/PA2/nn"+_postFix+".svg";
...
for (int t=0; t < 6 ; t++) // for every problem, the last AND is not a sub-problem 
 {
  solved = false;
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/7.25/andxorand_diif_st_points/PA2/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/7.25/andxorand_diif_st_points/PA2/nn"+_postFix+".dot");
    indiv.nn().write(ofs);

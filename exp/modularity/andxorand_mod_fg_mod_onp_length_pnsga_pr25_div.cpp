...
#define PR25
#define DIV
#line 1 "/home/henok/sferes/sferes2/trunk/exp/modularity/andxorand_mod.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
...
#else
      SFERES_ARRAY(float, obj_pressure, 1.0, 1.0, 1.0); //FG MOD DIV experiment: PR is not specified
#endif
    };
    struct rand_obj
    {
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
     std::string fname="/project/EvolvingAI/hmengist/7.25/andxorand_mod/mod100_1/mod100andequalsand/nn"+_postFix+".svg";
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/7.25/andxorand_mod/mod100_1/mod100andequalsand/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/7.25/andxorand_mod/mod100_1/mod100andequalsand/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
std::vector<float> inputs(8);
std::fstream sol;
sol.open("/project/EvolvingAI/hmengist/7.25/andxorand/PAsampling/runPA25_118/nn0336.txt",std::fstream::out | std::fstream::app);

...
#define PR100
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/combo_mod.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
 ...
 void eval(Indiv& ind)
     {
      ...
      //std::string fname="/project/EvolvingAI/hmengist/newmega/3hxorand3hxor/PA/log/nn"+_postFix+".svg";
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
      std::string fname="/project/EvolvingAI/hmengist/7.25/combo_mod/PA/nn"+_postFix+".svg";
...
void recording (I& indiv )
{
  ...
  int  nb_layers = Params::dnn::spatial::layers_size();
  simplayer=indiv.gen().get_layer();
  cout<<"this is the number of layers "<<simplayer.size()<<endl;
  ...
  for (size_t i = 1; i < nb_layers; ++i) //for each layer 
   {
      ...
      if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
	{
     ...
     //deb.open("/project/EvolvingAI/hmengist/7.25/combo_mod/comb_mod/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/7.25/combo_mod/PA/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/7.25/combo_mod/PA/nn"+_postFix+".dot");
    indiv.nn().write(ofs);

...
#define PR100
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/combevo_mod.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
 ...
 struct visualisation
    ...
    struct dnn
    {
     ...
     struct sampled
      ...
      struct spatial
      {
        SFERES_ARRAY(int, layers, 8, 4, 4, 2, 2, 1);
...
template<typename Indiv>
 void eval(Indiv& ind)
     {
      //std::cout<<"Evaluating the same problem"<<std::endl;
      ...
      for (size_t s = 0; s < k; ++s)
      {
         ...
         for (size_t i = 0; i < ind.gen().get_depth(); ++i)
            ind.nn().step(inputs);
            // behaviors
          std::vector<bool> andequals_res;
         for (size_t i = 0; i < Params::dnn::nb_inputs; i+=2)
          andequals_res.push_back(((bool)(inputs[i] > 0)) && ((bool)(inputs[i + 1] > 0)));
         for (size_t i = 0; i < Params::dnn::nb_outputs; ++i) 
            _behavior[s * Params::dnn::nb_outputs + i] = ind.nn().get_outf(i) > 0 ? 1 : 0;
             // errors
          andequals_res.push_back(!(andequals_res[0] ^ andequals_res[1]));
          andequals_res.push_back(!(andequals_res[2] ^ andequals_res[3]));
         if (andequals_res[4] && andequals_res[5])
...
#ifdef MOD
      this->_objs.push_back(1-_mod);//modularity discouraged
#endif
#ifdef HE
...
//	}
   ...
   //optimal length
      ...
      //std::string fname="/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PCC100/PCCcomb/nn"+_postFix+".svg";
...
void ans_subproblem(std::vector<float> inputs,std::vector<bool>& andequals_res)
  {
	 for (size_t i = 0; i < inputs.size(); i += 2)
          andequals_res.push_back(((bool)(inputs[i] > 0)) && ((bool)(inputs[i + 1] > 0)));            
          andequals_res.push_back(!(andequals_res[0] ^ andequals_res[1]));//andequals_res[4]
          andequals_res.push_back(!(andequals_res[2] ^ andequals_res[3]));//andequals_res[5]
          andequals_res.push_back((andequals_res[4] && andequals_res[5]));//andequals_res[6]
  }
template<typename I>
...
for (int t=0; t<7 ; t++) // every problem 
 {
  solved = false;
  for (size_t i = 1; i < nb_layers; ++i) //for each layer 
   {
            ...
            for (size_t s = 0; s < 256; ++s) 
             {
               ...
               for (size_t i = 0; i < indiv.gen().get_depth(); ++i)
                ...
                ans_subproblem(inputs,andequals_res);
       		values=std::make_pair(andequals_res[t],indiv.nn().get_graph()[v].get_current_output());
      ...
      if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
	{
       ...
       else
	   {
	    for (it=results.begin();it!=results.end();++it)
		   ...
		   break;
             ...
             if(!found && t==6)
		{
                    if ((indiv.nn().get_graph()[v].is_output()))
                     indiv.nn().get_graph()[v].set_solve(true,t);
                }
               else if (!found && t!=6)
                {
     ...
     //deb.open("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PA/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PCC100/PCCcomb/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PCC100/PCCcomb/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
               ...
               typename I::nn_t::vertex_desc_t A,typename I::nn_t::vertex_desc_t B,typename I::nn_t::vertex_desc_t com)
{
  ...
  std::vector<bool> andandequals_res;
...
tes.open("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PCC100/test.txt",std::fstream::out | std::fstream::app);
  ...
  for (size_t s = 0; s < 256; ++s)
   {
        ...
        tes<<s<<"\t";
        ans_subproblem(inputs,andandequals_res);
       ...
       values = make_pair(andandequals_res[sub],sum_AB);
       tes<<sum_AB<<"\t"<<andandequals_res[sub]<<"\n";

       testthresh.push_back(values);
       sum_AB = 0.0;
       andandequals_res.clear();    
   }
...
std::vector<float> inputs(8);
std::fstream sol;
sol.open("/project/evolutionaryRobotics/hmengist/7.25/combinationevo/PCC100/runPCC25_118/nn0336.txt",std::fstream::out | std::fstream::app);
...
public:
	...
	bool stop_criterion(const EA& ea) const {
		for (size_t i = 0; i < ea.pop().size(); ++i)
			if (ea.gen() >= 40 && ea.pop()[i]->fit().value() >= 0.8700)
				return true;
		return false;
    	}
    };

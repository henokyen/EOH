...
#define PR25
#define DIV
#line 1 "/home/hmengist/sferes/sferes2/trunk/exp/modularity/andequalsand.cpp"
#include <iostream>
#include <bitset>
#include<algorithm>
...
//	}
   ...
   //optimal length
      bool write_svg = (this->mode() == fit::mode::view);
      std::string fname="/project/evolutionaryRobotics/hmengist/7.25/andxorand/PCC100/PCCandequalsand/nn"+_postFix+".svg";
  ...
  for (size_t i = 1; i < nb_layers; ++i) //for each layer 
   {
      ...
      if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
	{
     ...
     //deb.open("/project/evolutionaryRobotics/hmengist/7.25/andequalsand/PA/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PCC100/PCCandequalsand/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/evolutionaryRobotics/hmengist/7.25/andxorand/PCC100/PCCandequalsand/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
tes.open("/project/evolutionaryRobotics/hmengist/7.25/andequalsand/PA/test.txt",std::fstream::out | std::fstream::app);
...
std::vector<float> inputs(8);
std::fstream sol;
sol.open("/project/evolutionaryRobotics/hmengist/7.25/andequalsand/PA/runPCC25_118/nn0336.txt",std::fstream::out | std::fstream::app);

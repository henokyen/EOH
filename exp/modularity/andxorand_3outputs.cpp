    ...
    struct dnn
    {
     ...
     struct sampled
      ...
      struct weight
      {
        struct sampled
        {
          SFERES_ARRAY(float, values, -2, -1, 1, 2);// weight are all 1 to experiment CCT on hierarchy
      ...
      static const size_t min_nb_neurons = 10; //that is 10 hidden nodes, so totally there will be 19 nodes 
      static const size_t max_nb_neurons = 10; //the same comment as above
      ...
      static const float m_rate_add_neuron = 0.05f; //Henok: increased the rate of adding neurons: complexification
      ...
      struct spatial
      {
        SFERES_ARRAY(int, layers, 8, 4, 6, 3);
...
int test=0;
      ...
      for (size_t s = 0; s < k; ++s)
      {
         ...
         for (size_t i = 0; i < Params::dnn::nb_inputs; i+=2)
          ...
          xorand_res.push_back(xorand_res[1] ^ xorand_res[2]);//5
          xorand_res.push_back(xorand_res[2] ^ xorand_res[3]);//6
         if (xorand_res[4] )
      ...
      if (xorand_res[5])
       ...
       if (xorand_res[6])
         err1 += ind.nn().get_outf(2) > 0 ? 0 : 1;
        else
        err1 += ind.nn().get_outf(2) > 0 ? 1 : 0;                                                                                                                                                                  
     //behaviour
 for (size_t i = 0; i < Params::dnn::nb_outputs; ++i)
      ...
      //typename Indiv::nn_t::graph_t g =ind.gen().get_graph();
      //std::cout<<"gsimp "<<boost::num_vertices(gsimp)<<std::endl;
      //std::cout<<"g "<<boost::num_vertices(g)<<std::endl;
      ...
      std::string fname="/project/EvolvingAI/hmengist/3outputs/nn"+_postFix+".svg";
     ...
     //_optlength=_length;
      ...
      if (this->mode() == sferes::fit::mode::view)
      {
        ...
        //std::ofstream ofs("/project/EvolvingAI/hmengist/mega/3hxor/PA/log/nn"+_postFix+".dot");
      //if ((_grc >=0.75000)&&(1-err1/norm)==1)
         //recording(ind);
      //else
	//{
	//std::fstream fsol;
        //fsol.open("/project/EvolvingAI/hmengist/7.25/xorands/PA/solvent.txt",std::fstream::out | std::fstream::app);
        //fsol <<"nn"+_postFix<<" "<<0<<"\n";
        //fsol.close();
//	}
        recording(ind);
        ...
        std::map<std::string, std::pair<double,int> > neuron_coords;
        //ind.gen().visualize(x,y,neuron_coords,(1-err1/norm));
      }
  
   //optimal length
    //bool write_svg = (this->mode() == fit::mode::view);
    //std::string fname="/project/EvolvingAI/hmengist/3outputs/PA/nn"+_postFix+".svg";
...
void ans_subproblem(std::vector<float> inputs,std::vector<bool>& xorand_res)
  {
	 for (size_t i = 0; i < inputs.size(); i += 2)
          ...
          xorand_res.push_back(xorand_res[1] ^ xorand_res[2]);//xorand_res[5]
          xorand_res.push_back(xorand_res[2] ^ xorand_res[3]);//xorand_res[6]
  }
template<typename I>
...
for (int t=0; t < 4 ; t++) // every problem
 {
  solved = false;
  ...
  if (check_threshold(threshold))  // if there is a threshold..if t is solved by current v
    ...
    if(!found && t==6)
     ...
     //deb.open("/project/EvolvingAI/hmengist/7.25/andxorand/PCCsampling/debug.txt",std::fstream::out | std::fstream::app);
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.open("/project/EvolvingAI/hmengist/3outputs/PA/solvent.txt",std::fstream::out | std::fstream::app);
    sol <<"nn"+_postFix<<" "<<results.size()/number_problems<<"\n";
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/3outputs/PA/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
...
tes.open("/project/EvolvingAI/hmengist/7.25/andxorand/PCCsampling/test.txt",std::fstream::out | std::fstream::app);
...
std::vector<float> inputs(8);
std::fstream sol;
sol.open("/project/EvolvingAI/hmengist/7.25/andxorand/PCCsampling/runPCC25_118/nn0336.txt",std::fstream::out | std::fstream::app);
...
int main(int argc, char **argv)
{
  ...
  typedef phen::Dnn<gen_t, FitXorXor<Params>, Params> phen_t;

  ...
  struct Params
  {
    ...
    struct dnn
    {
     ...
     struct sampled
      ...
      struct spatial
      {
        SFERES_ARRAY(int, layers, 8, 4, 4, 2, 2);
...
int test=0;
      ...
      for (size_t s = 0; s < k; ++s)
      {
       ...
       if (xorand_res[4] && xorand_res[5])
          ...
          err1 += ind.nn().get_outf(0) > 0 ? 1 : 0;                                                                                                            if (and_res[2] ^ and_res[3])

       if (xorand_res[4] || xorand_res[5])
          err1 += ind.nn().get_outf(0) > 0 ? 0 : 1;
       else
          err1 += ind.nn().get_outf(0) > 0 ? 1 : 0;                                                                                                                                                              err1 += ind.nn().get_outf(1) > 0 ? 0 : 1;

  for (size_t i = 0; i < Params::dnn::nb_outputs; ++i)
            _behavior[s * Params::dnn::nb_outputs + i] = ind.nn().get_outf(i) > 0 ? 1 : 0;                                                                                                                                                               else
      ...
      //std::string fname="/project/EvolvingAI/hmengist/nn"+_postFix+".svg";
...
for (it =results.begin();it!=results.end();++it)
    ...
    sol.close();
    std::ofstream ofs("/project/EvolvingAI/hmengist/PA/nn"+_postFix+".dot");
    indiv.nn().write(ofs);
